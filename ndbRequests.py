...
class GetUniversities(webapp2.RequestHandler):
    def get(self):
        ...
        print(flag)
        if town == '':
            universities =  enteries_sets.University.query().fetch(count)
        else:
            if flag:
                universities = enteries_sets.University.query(enteries_sets.University.town == town).fetch(limit=count, offset=offset)
            else:
                universities = enteries_sets.University.query(enteries_sets.University.town == town and
        ...
        for university in universities:
            if university.image_url is None and university.logo_url is None:
                university.get_images()
        universities = [p.to_dict() for p in universities]
...
class DropDataBase(webapp2.RequestHandler):
    def get(self):
        ...
        university_speciality = enteries_sets.UniversitySpeciality.query().fetch()
        ...
        for speciality in university_speciality:
